...
import static com.youngliving.ui.BatchVersionsPage.*;
...
public class Select implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        utils.wait(1001);
        actor.attemptsTo(
                WaitUntil.the(TXT_BATCH_APPLICATION, isClickable()).forNoMoreThan(15).seconds(),
                Click.on(TXT_BATCH_APPLICATION),
                Enter.theValue(batchApp).into(TXT_BATCH_APPLICATION)
        );

        System.out.println(BLUE + INDENTATION + "VersionTitle:  " + RESET + versionTitle);
        actor.attemptsTo(
                WaitUntil.the(ICON_FIND, isClickable()).forNoMoreThan(15).seconds(),
                Click.on(ICON_FIND),
                Click.on(CHECKBOX_MEMPHIS.of(batchVersion)),
                Click.on(ICON_GREEN_CHECK)
        );


        System.out.println(BLUE + INDENTATION + "BatchApp:      " + RESET + batchApp);
        actor.attemptsTo(
                WaitUntil.the(CHECKBOX_DATA_SELECTION, isClickable()).forNoMoreThan(15).seconds(),
                Click.on(CHECKBOX_DATA_SELECTION),
                Click.on(ICON_SUBMIT)
        );
        System.out.println(BLUE + INDENTATION + "BatchVersion:  " + RESET + batchVersion);
        utils.wait(1001);
    }
}
